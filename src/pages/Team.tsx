      ...
      <CardContent className="p-4 sm:p-6 text-center">
        <div className="relative mb-3 sm:mb-4">
          <img
            ...
            alt={p.name}
            className={`mx-auto object-cover rounded-full border-2 sm:border-4 border-primary/20 group-hover:border-primary/40 transition-colors duration-300 ${isHead ? 'w-24 h-24 sm:w-32 sm:h-32 lg:w-36 lg:h-36' : 'w-20 h-20 sm:w-28 sm:h-28 lg:w-32 lg:h-32'}`}
            onError={(e) => {
              ...
              <div className="flex gap-2 sm:gap-3">
                {p.email && (
                  <button
                    ...
                    className="p-1.5 sm:p-2 bg-white/20 backdrop-blur-sm rounded-full hover:bg-white/30 transition-colors duration-200"
                    title={`Email ${p.name}`}
                  >
                ...
                {p.linkedin && (
                  ...
                  >
                    <Linkedin className="h-3 w-3 sm:h-4 sm:w-4 lg:h-5 lg:w-5 text-white" />
                  </button>
                )}
              </div>
            </div>
        ...
        <h3 className="text-sm sm:text-lg lg:text-xl font-semibold text-foreground mb-1">{p.name}</h3>
        {p.role && <p className="text-xs sm:text-sm text-muted-foreground mb-2">{p.role}</p>}
        {p.about && <p className="text-xs sm:text-sm text-muted-foreground">{p.about}</p>}
      </CardContent>
    </Card>
  );
          ...
          <div className="container mx-auto px-3 sm:px-4 lg:px-6">
            {sections.map((section) => (
              <div key={section.title} className="mb-12 sm:mb-16">
                <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-center mb-6 sm:mb-8 text-foreground">{section.title}</h2>
                {section.kind === 'single' && (
                  <div className="max-w-xs sm:max-w-md mx-auto">{renderCard(section.person, true)}</div>
                )}
                {section.kind === 'pair' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 max-w-3xl mx-auto justify-items-center">
                ...
                {section.kind === 'headChildren' && (
                  <div className="max-w-5xl mx-auto">
                    <div className="mb-6 sm:mb-8 max-w-xs sm:max-w-md mx-auto">{renderCard(section.head, true)}</div>
                    <div
                      className={`grid gap-4 sm:gap-6 lg:gap-8 justify-items-center mx-auto ${
